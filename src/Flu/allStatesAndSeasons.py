...
from openpyxl.styles import Border, Side, PatternFill, Font, GradientFill, Alignment, colors
from openpyxl import Workbook
from ComputeStandardDevs import aveSDs
import math
import collections
import copy
import csv
...
def computeMostCommon(fasta, Season):
    ...
    most_common__seq = ""
    for el in colEls:
        most_common__seq += ((collections.Counter(el).most_common(1)[0])[0])

    return (most_common__seq, return_fasta)
        ...
        if row[0] == '>': ##indicates the start of a new strand
            ...
            for el in row:
                if el != "-" and el != '\n' and numElements < 1699: ##stops after the correct number of elements
                    color = False
                    if getVal(el) == -100:
                        el = correctSequence[colNum-1] ##if the value is not in our hydropathy table, this will set it to the most common value and color it
                        color = True
                        Xs.add(sheet1['A' + str(rowNum)].value)
                    if colNum+1 in nSites: ##changes n's to z's where appropriate
                        colNum += 1
                        numElements += 1
...
def compBranch(Reader, numberOfRows, offSet):
    ...
    for row in Reader: ##reads each row of the csv of the distance matrix created by MEGA7
        colNum = 0
        t = (', '.join(row))
        mylist = t.split(",")
...
def compSDs(book, SDoffSet, offSet, years, State):
    ...
    while(Next):
        ...
        if el.value == None: ##this is only true when colNum reaches the end of a sequence
            if (rowNum-row1) > 1:
                totalVal += 100 ##since el.value is None, gelVal will return -100 and it needs to be corrected for here
...
def getVal(el): #hydropathy scores
    if el == 'A':
        return 68.0
    elif el == 'C':
        return 73.3
...
def allStatesAndSeasons():
    book = Workbook()
    Season = input("Season (Ex: 14_15): ")

    StateString = ''

    StateOne = input("State for analysis: ")
    States = {StateOne}
    StateString += StateOne
    while(True):
        another = input('Another state? (Y/N)')
        if another != 'Y': break
        else:
            StateString += ','
            StateNext = input("State for analysis: ")
            States.add(StateNext)
            StateString += StateNext
    ...
    for num in range(0, numSeasons):
        year1 = int(Season[:2])+ num
        year2 = int(Season[3:])+ num
    ...
    aveSDs(book,SDOffSet)
        
    print('Saving...')
    book.save("U:\\ResearchData\\rdss_hhaim\\LAB PROJECTS\\Influenza\\H3N2 Analyses\\USA\\" + StateString + ".xlsx")
