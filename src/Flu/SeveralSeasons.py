...
# seasons and compare difference across seasons
################################################
...
from ComputeStandardDevs import ComputeStandardDevs, getVal
...
def allSeasons():
    ...
    Season = input("First Season (Ex: 14_15): ")
    ...
    for num in range(0, numSeasons):
        ...
        fasta2 = open("U:\\ResearchData\\rdss_hhaim\\LAB PROJECTS\\Influenza\\H3N2 Analyses\\USA\\" + State+ "\\"+ years +'\\' + State + Season1 + '-PRO.fasta', "r")
        ...
        (rowNum, Nucs, Names) = AnalyzeNucleotides(Season2, fastaNuc)
        offSetSave = max(rowNumbers) + 2
        (rowNumbers, numBranches) = ComputeBranches(Reader, rowNum, offSetSave)
        (correctSeq, fasta2) = ComputeMostCommon(fasta2, Season2)
        zSites = findZSites(correctSeq)
        (offSet, Xs) = CreateExcelSheetsForProteins(book,Season2, correctSeq, fasta2, rowNumbers, zSites, offSet)
        SDOffSet = ComputeStandardDevs(book, SDOffSet,offSetSave, years, State)
        offSet += 2
        for x in Xs:
            print(x)

    print('Saving...')
    book.save("U:\\ResearchData\\rdss_hhaim\\LAB PROJECTS\\Influenza\\H3N2 Analyses\\USA\\"+State+"\\Across-Seasons-" + State+".xlsx")
